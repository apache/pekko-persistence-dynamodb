...
class SnapshotStoreTckSpec extends CopySnapshotStoreSpec(
  ConfigFactory.load()
) with DynamoDBUtils {
...
// a temporary copy of the akka tck spec for debugging purposes
abstract class CopySnapshotStoreSpec(config: Config) extends PluginSpec(config)
